...
| Assembly departments (Works; Environmental Health and Sanitation, since municipal assemblies have no standalone Waste Management Department) | Direct users (Reports Officer, Field Crew supervisors). | Workflow efficiency, evidence base, departmental accountability. |
...
| FR-007 | The system shall allow a Citizen to delete their account, with reports submitted by that account anonymised rather than deleted, preserving the operational record. | Should | Data-protection principle. *(not implemented: no delete-account action exists in the Profile screen or a backing edge function; verified by code audit, see Testing_Report.pdf §5.2)* |
...
| FR-016 | The system shall permit the Citizen to attach up to five photographs to a single report. | Should | Richer evidence. *(revised in v2.0: the delivered cap is five, enforced identically client-side (`ReportFlow.tsx`) and server-side (`submit-report`), not three as v1.0 specified; verified by code audit, see Testing_Report.pdf §5.2)* |
...
| FR-023 | The system should allow a Citizen to add a follow-up comment to a report while it is open. | Should | Two-way communication. Not implemented; verified by code audit, see Testing_Report.pdf §5.2. |
...
| FR-033 | The public map shall be accessible without authentication. | Should | Public good; **not implemented as specified**: the citizen app's Map route is currently wrapped in a `RequireAuth` guard and redirects an unauthenticated visitor to sign-in, the opposite of this requirement. Verified by code audit, see Testing_Report.pdf §5.2. |
...
| FR-054 | The system should allow reassignment of a report from one crew to another by the Reports Officer or Administrator. | Should | Operational reality; **not implemented**: the Assign action is only legal from Submitted, Acknowledged, or Reopened, both client-side (`reportActions.ts`) and server-side (`transition-report`), so a report cannot move from one crew to another once it is Assigned. Verified by code audit, see Testing_Report.pdf §5.2. |
...
| FR-073 | The system shall support in-application notifications (via real-time subscription) and email notifications. SMS is deferred to future enhancements. | Must | Channel baseline. *(revised: in-app delivery mechanism now named Supabase Realtime)* |
...
| FR-085 | The Administrator should be able to export data in CSV format. | Should | Reporting and backup; **not implemented**: no CSV export exists anywhere in the console. Verified by code audit, see Testing_Report.pdf §5.2. |
...
| ID | Requirement | Priority | Source / Rationale |
|---|---|---|---|
| FR-090 | The system should present a management dashboard showing total reports submitted, total resolved, current backlog, and average time to resolution. | Should | Management view; **partially implemented**: the Analytics screen shows total reports, open backlog, and average resolution time, but not an all-time total-resolved count (only a resolved-this-week figure). Verified by code audit, see Testing_Report.pdf §5.2. |
| FR-091 | The system should present a hotspot view identifying geographic clusters of reports. | Should | Resource planning; **not implemented**: the Map screen plots individual markers only, with no clustering or density view. Verified by code audit, see Testing_Report.pdf §5.2. |
| FR-092 | The system should present per-crew productivity accessible to Administrators only. | Should | Operational management; **not implemented**: the Crews screen shows only a member count per crew, no assigned/in-progress/resolved counts or average resolution time. Verified by code audit, see Testing_Report.pdf §5.2. |
