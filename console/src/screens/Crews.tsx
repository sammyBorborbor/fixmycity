...
import type { Crew } from '../lib/store.tsx';
...
export default function Crews() {
  const { crews, reports, addCrew, toggleCrewAvailability, addMember, removeMember, setLead } = useStore();
  ...
  function submit() {
    ...
    addCrew({ name: form.name.trim(), dept: form.dept, lead: lead || '—', phone: '—', available: true, roster: lead ? [lead] : [] });
    setForm({ name: '', dept: 'Sanitation', lead: '' });
    setAdding(false);
  }

  function addMemberTo(id: string) {
    ...
    <div className="p-6 max-w-[1100px]">
      ...
      {adding && (
        <div className="bg-white rounded-xl ring-1 ring-black/5 shadow-sm p-5 mb-5 fade-in">
          ...
          <p className="text-[11px] text-muted mt-2">The team lead becomes the first member. Add the rest from the crew card.</p>
        ...
        {crews.map(c => {
          ...
          return (
            <div key={c.id} className="bg-white rounded-xl ring-1 ring-black/5 shadow-sm p-5 flex flex-col">
              ...
              {/* manage members */}
              <button onClick={() => { setOpenId(open ? null : c.id); setNewMember(''); }}
              ...
              {open && (
                <div className="mt-3 fade-in">
                  <div className="flex flex-col gap-1.5">
                    {roster.length === 0 && <p className="text-xs text-muted py-1">No members yet. Add the first one below.</p>}
                    {roster.map(m => {
                      const isLead = m === c.lead;
                      return (
                        <div key={m} className="flex items-center gap-2 bg-gray-50 rounded-lg px-2.5 py-1.5">
                          <span className="w-6 h-6 rounded-full bg-ocean/10 text-ocean text-[10px] font-bold flex items-center justify-center shrink-0">
                            {m.trim().split(/\s+/).map(x => x[0]).slice(0, 2).join('').toUpperCase()}
                          </span>
                          <span className="text-sm text-ink truncate flex-1">{m}</span>
                          {isLead ? (
                            <span className="text-[10px] font-semibold text-gold bg-gold/10 px-1.5 py-0.5 rounded shrink-0">Lead</span>
                          ) : (
                            <button onClick={() => setLead(c.id, m)} title="Make lead"
                              className="text-[11px] font-semibold text-muted hover:text-ocean shrink-0">Make lead</button>
                          )}
                          <button onClick={() => removeMember(c.id, m)} title="Remove member"
                            className="text-gray-400 hover:text-red-500 shrink-0"><Icon name="X" size={15} /></button>
                        </div>
                      );
                    })}
                  </div>
                  <div className="flex gap-2 mt-2.5">
                    <input value={newMember} onChange={e => setNewMember(e.target.value)}
                      onKeyDown={e => { if (e.key === 'Enter') addMemberTo(c.id); }}
                    ...
                    <Btn variant="primary" size="sm" icon="UserPlus" onClick={() => addMemberTo(c.id)}>Add</Btn>
                  </div>
                </div>
              )}
